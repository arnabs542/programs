...
public class LevenshteinDistance {
    ...
    public static void main(String[] args) {
        // Recursive
        System.out.println(recur("abc".toCharArray(), 0, "abc".toCharArray(), 0));  // 0
        System.out.println(recur("a".toCharArray(), 0, "abc".toCharArray(), 0));  // 2
        System.out.println(recur("kitten".toCharArray(), 0, "sitting".toCharArray(), 0));  // 3

        // DP
        System.out.println(dp("ab".toCharArray(), "abc".toCharArray()));  // 1
        System.out.println(dp("kitten".toCharArray(), "sitting".toCharArray()));  // 3

        // iterating reverse eases base cases
        System.out.println(dp_rev("kitten", "sitting", "kitten".length(), "sitting".length()));    // 3
    }
     ...
     *         +/ -|  \replace  ... bf = 3
     *        aa   .   b
     * Runtime => if depth of rec = n which is length of string, then its O(3^n) exponential
     */
    static int recur(char[] s1, int i, char[] s2, int j) {
        ...
        if (s1[i] == s2[j]) return recur(s1, i+1, s2, j+1);

        // first char differs, then try all options for corrections i.e. add, delete, replace
        int delete = recur(s1, i+1, s2, j); // just shift s1's ptr
        int add = recur(s1, i, s2, j+1);    // n, wn => (w)n, wn => we just move j's ptr by 1 as we assume after adding char, it matches
        int replace = recur(s1, i+1, s2, j+1);  // assume they are equal, move on
     ...
     * DP Formula => dp[i][j] = 1 + min (dp[i][j-1],
     *                                   dp[i-1][j],
     *                                   dp[i-1][j-1]) , if chars don't match
     *                        = dp[i-1][j-1] , if chars match
     * Runtime = O(n * m) polynomial
     */
    ...
    static int dp_rev(String str1, String str2, int m, int n) {
        ...
        if (str1.charAt(m-1) == str2.charAt(n-1))
            return dp_rev(str1, str2, m-1, n-1);

        // If last characters are not same, consider all three
        // operations on last character of first string, recursively
        // compute minimum cost for all three operations and take
        // minimum of three values.
        return 1 + min ( dp_rev(str1,  str2, m, n-1), // Insert
                dp_rev(str1,  str2, m-1, n),         // Remove
                dp_rev(str1,  str2, m-1, n-1)    // Replace
        );
    }
